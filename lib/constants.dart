const appName = 'AOE2 Build Orders';
const arrow = '→';

class Strings {
  const Strings._();

  // Resources
  static final sheep = 'Sheep';
  static final boar = 'Boar';
  static final berries = 'Berries';
  static final farms = 'Farms';
  static final wood = 'Wood';
  static final gold = 'Gold';
  static final stone = 'Stone';

  // Buildings
  static final mill = 'Mill';
  static final lumberCamp = 'Lumber Camp';
  static final miningCamp = 'Mining Camp';
  static final barracks = 'Barracks';
  static final archeryRange = 'Archery Range';
  static final stable = 'Stable';
  static final market = 'Market';
  static final blacksmith = 'Blacksmith';
  static final townCenter = 'Town Center';

  // Technologies
  static final loom = 'Loom';
  static final wheelbarrow = 'Wheelbarrow';
  static final doubleBitAxe = 'Double-Bit Axe';
  static final horseCollar = 'Horse Collar';
  static final goldMining = 'Gold Mining';
  static final feudalAge = 'Feudal Age';
  static final castleAge = 'Castle Age';
  static final bloodlines = 'Bloodlines';
  static final scaleBardingArmor = 'Scale Barding Armor';
  static final fletching = 'Fletching';

  // Units
  static final militia = 'Militia';
  static final manAtArms = 'Man-at-Arms';
  static final archers = 'Archers';
  static final scouts = 'Scouts';
  static final crossbowmen = 'Crossbowmen';
  static final knights = 'Knights';
  static final monks = 'Monks';

}
